...
import { useGetAgentStatsQuery } from "@/redux/api/adminApi";
...
interface AgentStats {
  ...
  total_agents: number;
  message_overview: { date: string; count: number }[];
}

const AgentDashboardPage = () => {
  const { setPageSubtitle, setPageTitle } = useDashboard();
  const { setLoading, setLoadingText } = useLoading();
  const { data: statsData, isLoading: statsLoading, error: statsError } = useGetAgentStatsQuery(undefined);
  ...
  const stats = statsData?.success ? [
    ...
    { icon: <Tag size={24} />, label: "Total Messages", value: statsData.message_overview.reduce((sum: number, item: any) => sum + item.count, 0) || 0 },
    { icon: <Users size={24} />, label: "Today Messages", value: statsData.message_overview.find((item: any) => item.date === "2025-08-30")?.count || 0 },
  ] : [];

  const chartData = statsData?.success && statsData.message_overview
    ? statsData.message_overview.map((item:any) => item.count)
    : [];
  const chartLabels = statsData?.success && statsData.message_overview
    ? statsData.message_overview.map((item:any) => new Date(item.date).toLocaleDateString('en-US', { weekday: 'short' }))
    : [];
